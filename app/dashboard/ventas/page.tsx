...
const PRODUCTOS_DISPONIBLES: ProductoVenta[] = [
  ...
  {
    id: 2,
    nombre: "Barbie Edición Especial",
    precio: 150,
    stock: 10,
    category: "Muñecas",
    sku: "MUN-002",
...
export default function VentasPage() {
  ...
  const productosFiltrados = PRODUCTOS_DISPONIBLES.filter(
    (p) =>
    ...
    setCarrito((prev) =>
      prev.map((item) => {
        if (item.id === id) {
          ...
          const productoOriginal = PRODUCTOS_DISPONIBLES.find(
            (p) => p.id === id
          );
  ...
  const descuento = clienteSeleccionado?.tipo === "Vip" ? subtotal * 0.1 : 0;
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-start w-full mb-1">
                    ...
                    <span
                      className={`text-[10px] lg:text-xs font-mono font-medium ${
                        prod.stock < 3 ? "text-red-500" : "text-slate-400"
                      }`}
                    >
                      Stock: {prod.stock}
                    </span>
